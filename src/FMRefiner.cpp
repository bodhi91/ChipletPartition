...
void ChipletRefiner::BuildChiplets(const HGraphPtr &hgraph) {
  ...
  for (int i = 0; i < hgraph->GetNumHyperedges(); ++i) {
    ...
    bundled_nets_.push_back(BundledNet(std::pair<int, int>(term_a, term_b),
                                       weight, reach, io_area));
  }
...
ChipletRefiner::Floorplanner(int max_steps, int perturbations,
  ...
  std::shared_ptr<SACore> best_sa;
  std::vector<std::shared_ptr<SACore>>
  ...
  for (int i = 0; i < num_threads_; i++) {
    ...
    std::shared_ptr<SACore> sa = std::make_shared<SACore>(
...
bool ChipletRefiner::CheckFloorPlanFeasible(const HGraphPtr hgraph,
  ...
  BuildChiplets(chiplet_level_netlist);
  auto orig_pos_seq = local_pos_seq_;
  auto orig_neg_seq = local_neg_seq_;
  if (chiplet_level_netlist->GetNumVertices() < local_pos_seq_.size() ||
      chiplet_level_netlist->GetNumVertices() < local_neg_seq_.size()) {
    ClearLocalSequences();
    // find which pair id is not in the sequence
    for (int i = 0; i < orig_pos_seq.size(); ++i) {
      if (orig_pos_seq[i] < chiplet_level_netlist->GetNumVertices()) {
        local_pos_seq_.push_back(orig_pos_seq[i]);
      }
      if (orig_neg_seq[i] < chiplet_level_netlist->GetNumVertices()) {
        local_neg_seq_.push_back(orig_neg_seq[i]);
      }
    }
  } else if (chiplet_level_netlist->GetNumVertices() > local_pos_seq_.size() ||
             chiplet_level_netlist->GetNumVertices() > local_neg_seq_.size()) {
    ClearLocalSequences();
    for (int i = 0; i < chiplet_level_netlist->GetNumVertices(); ++i) {
      local_pos_seq_.push_back(i);
      local_neg_seq_.push_back(i);
    }
...
PyObject *ChipletRefiner::BuildModel(const std::vector<int> &partitionIDs,
  ...
  std::vector<int> partition_ids_cont = partitionIDs;

  // Sort the partition IDs (no parallelism needed for sort due to dependencies)
  std::sort(partition_ids_cont.begin(), partition_ids_cont.end());

  // Remove duplicates to create unique partition IDs
  std::vector<int> partition_ids_cont_unique = partition_ids_cont;
  partition_ids_cont_unique.erase(std::unique(partition_ids_cont_unique.begin(),
                                              partition_ids_cont_unique.end()),
                                  partition_ids_cont_unique.end());

  // Create a map to map the original partition IDs to contiguous IDs
  std::map<int, int> partition_id_map;

// Parallelizing the creation of partition_id_map
#pragma omp parallel for
  for (int i = 0; i < partition_ids_cont_unique.size(); ++i) {
#pragma omp critical // Ensure the map is updated safely in parallel
    partition_id_map[partition_ids_cont_unique[i]] = i;
  }

// Update the partition_ids_cont to contiguous partition IDs
#pragma omp parallel for
  for (int i = 0; i < partition_ids_cont.size(); ++i) {
    partition_ids_cont[i] = partition_id_map[partition_ids_cont[i]];
  }

  // Create new vectors for contiguous tech_array, aspect_ratios, and locations
  std::vector<std::string> tech_array_cont(partition_ids_cont_unique.size());
  std::vector<float> aspect_ratios_cont(partition_ids_cont_unique.size());
  std::vector<float> x_locations_cont(partition_ids_cont_unique.size());
  std::vector<float> y_locations_cont(partition_ids_cont_unique.size());

// Populate the new continuous arrays
#pragma omp parallel for
  for (int i = 0; i < partition_ids_cont_unique.size(); ++i) {
    int mapped_id = partition_id_map[partition_ids_cont_unique[i]];
    tech_array_cont[i] = tech_array[mapped_id];
    aspect_ratios_cont[i] = aspect_ratios[mapped_id];
    x_locations_cont[i] = x_locations[mapped_id];
    y_locations_cont[i] = y_locations[mapped_id];
  }

  /*int num_parts =
   *std::max_element(partitionIDs.begin(), partitionIDs.end()) + 1;*/

  int num_parts = partition_ids_cont_unique.size();

  // Get a list of block numbers for each partition.
  std::vector<std::vector<int>> partitionVector =
      std::vector<std::vector<int>>(num_parts, std::vector<int>());

  for (int blockId = 0; blockId < partition_ids_cont.size(); blockId++) {
    partitionVector[partition_ids_cont[blockId]].push_back(blockId);
  }
  ...
  if (Py_IsInitialized()) {
    ...
    if (pModule != nullptr) {
      ...
      if (PyCallable_Check(pFunction)) {
        // Convert C++ vectors to Python lists
        PyObject *pTechList = PyList_New(tech_array_cont.size());
        PyObject *pAspectRatioList = PyList_New(aspect_ratios_cont.size());
        PyObject *pXLocationList = PyList_New(x_locations_cont.size());
        PyObject *pYLocationList = PyList_New(y_locations_cont.size());
        PyObject *pPowerList = PyList_New(chiplet_powers.size());
        PyObject *pCoreAreaList = PyList_New(chiplet_areas.size());
        if (PyErr_Occurred()) {
          std::cout << "Python error occurred" << std::endl;
        ...
        for (size_t i = 0; i < chiplet_powers.size(); i++) {
          PyObject *pTech = PyUnicode_DecodeUTF8(
              tech_array_cont[i].c_str(), tech_array_cont[i].size(), "strict");
          PyObject *pAspectRatio = PyFloat_FromDouble(aspect_ratios_cont[i]);
          PyObject *pXLocation = PyFloat_FromDouble(x_locations_cont[i]);
          PyObject *pYLocation = PyFloat_FromDouble(y_locations_cont[i]);
          PyObject *pPower = PyFloat_FromDouble(chiplet_powers[i]);
          PyObject *pCoreArea = PyFloat_FromDouble(chiplet_areas[i]);
          PyList_SetItem(pTechList, i, pTech);
          PyList_SetItem(pAspectRatioList, i, pAspectRatio);
        ...
        PyObject *pArgs = PyTuple_Pack(
            7, pTechList, pAspectRatioList, pXLocationList, pYLocationList,
            pPowerList, pCoreAreaList, PyLong_FromLong(num_parts));
        PyObject *chip_params = PyObject_CallObject(pFunction, pArgs);
        if (PyErr_Occurred()) {
          std::cout << "Python error occurred" << std::endl;
          PyErr_Print();
...
HGraphPtr ChipletRefiner::GenerateNetlist(const HGraphPtr hgraph,
  ...
  // make the partition ids continuous
